...
import numpy as np
from fastapi_service.schemas import Strategy_Input,Rsi_Input
import os
import pandas as pd
import plotly.express as px  # Import Plotly Express for charting
from backtest.metrics import calculate_sharpe, calculate_maxdrawdown
import time
from fastapi_service.logic import moving_average_implementation, rsi_implementation  # Ensure your package structure supports this
...
async def strategy_endpoint(input: Strategy_Input):
    try:
        # Call your helper function that handles data processing and saves CSV.
        csv_path = moving_average_implementation(input)
        # Build a redirect URL with the query parameters extracted from the input
        redirect_url = f"/result/?symbol={input.symbol}&short_window={input.short_window}&long_window={input.long_window}"
